...
export default function ClearanceManagement() {
  ...
  const filteredClearances = clearanceRecords.filter((record) => {
    ...
    const matchesStatus =
      statusFilter === "all" || record.clearanceStatus === statusFilter;
    ...
    return (
      matchesSearch && matchesStatus && matchesDepartment && matchesLwdRange
    );
  });
  ...
  const processBulkAction = () => {
    ...
    selectedClearances.forEach((clearanceId) => {
      const clearanceIndex = clearanceRecords.findIndex(
        (c) => c.id === clearanceId,
      );
      if (clearanceIndex !== -1) {
        switch (action) {
          case "approve":
            ...
            clearanceRecords[clearanceIndex].clearanceDate = new Date()
              .toISOString()
              .split("T")[0];
            break;
          case "reminder":
        ...
        clearanceRecords[clearanceIndex].lastUpdated = new Date()
          .toISOString()
          .split("T")[0];
      }
    });
  ...
  const ClearanceDetailsDialog = ({ record }: { record: ClearanceRecord }) => (
    <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription>
          Employee ID: {record.employeeId} | Clearance ID: {record.id}
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div>
                <span className="font-medium">Name:</span> {record.employeeName}
              </div>
              <div>
              ...
              <div>
                <span className="font-medium">Manager:</span> {record.manager}
              </div>
              <div>
                <span className="font-medium">Priority:</span>
                <Badge className={`ml-2 ${getPriorityColor(record.priority)}`}>
                  {record.priority}
                </Badge>
              </div>
            </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {assetClearances
              .filter((ac) => ac.employeeId === record.employeeId)
              .map((asset) => (
                <div
                  key={asset.id}
        ...
        <div>
          ...
          <div className="space-y-2">
            {offboardingTasks
              .filter((task) => task.employeeId === record.employeeId)
              .map((task) => (
                <div
                  key={task.id}
                  className="flex items-center justify-between p-3 border rounded-lg"
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">
              Handle employee offboarding and asset clearance
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="outline">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">
                    {
                      clearanceRecords.filter(
                        (c) => c.clearanceStatus === "In Progress",
                      ).length
                    }
                  </p>
                </div>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>
                    Clearance Records ({filteredClearances.length})
                  </CardTitle>
                  <CardDescription>
                    Filter and manage employee clearance processes
                  </CardDescription>
                </div>
                    ...
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            className={getStatusColor(record.clearanceStatus)}
                          >
                            {record.clearanceStatus}
                          </Badge>
                        </div>
                      ...
                      <TableCell>
                        <div className="space-y-1">
                          <div className="flex justify-between text-sm">
                            <span>
                              {record.assetsReturned}/{record.assetsAllocated}
                            </span>
                            <span>{record.clearanceProgress}%</span>
                          </div>
                          <Progress
                            value={record.clearanceProgress}
                            className="h-1"
                          />
                        </div>
                      </TableCell>
        ...
        <TabsContent value="assets" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Monitor and manage asset returns during clearance process
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                ...
                <TableBody>
                  {assetClearances.map((asset) => {
                    const employee = clearanceRecords.find(
                      (r) => r.employeeId === asset.employeeId,
                    );
                    return (
        ...
        <TabsContent value="bulk" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Process multiple clearances using templates and bulk operations
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                  ...
                  <Card>
                    <CardContent className="p-4">
                      ...
                      <p className="text-sm text-gray-600 mb-4">
                        Upload a CSV file with clearance data to process
                        multiple records
                      </p>
                      <div className="space-y-2">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        <div className="font-medium">
                          Bulk approval - Q1 2024 clearances
                        </div>
                        <div className="text-sm text-gray-500">
                          Processed 15 records • 2024-01-25
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        <div className="font-medium">
                          Template upload - IT Department clearances
                        </div>
                        <div className="text-sm text-gray-500">
                          Processed 8 records • 2024-01-20
                        </div>
                      </div>
                      <Badge className="bg-green-100 text-green-800">
                        Completed
        ...
        <TabsContent value="workflow" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Manage complete employee offboarding process with automated
                tasks
              </CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <div className="space-y-3">
                        <div className="p-3 border rounded-lg">
                          ...
                          <p className="text-sm text-gray-500">
                            Asset return + Access revocation + Documentation
                          </p>
                        ...
                        <div className="p-3 border rounded-lg">
                          ...
                          <p className="text-sm text-gray-500">
                            Extended workflow with knowledge transfer
                          </p>
                          <div className="flex gap-2 mt-2">
                            <Button size="sm" variant="outline">
                              Edit
                        ...
                        <div className="p-3 border rounded-lg">
                          ...
                          <p className="text-sm text-gray-500">
                            Includes shipping and remote asset collection
                          </p>
                          <div className="flex gap-2 mt-2">
                            <Button size="sm" variant="outline">
                              Edit
      ...
      <Dialog open={showBulkModal} onOpenChange={setShowBulkModal}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select an action to perform on {selectedClearances.length}{" "}
              selected clearance records
            </DialogDescription>
          </DialogHeader>
