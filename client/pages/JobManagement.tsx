...
import { Search, Filter, Plus, Eye, Edit, Trash2, Calendar, Users, Clock, CheckCircle, AlertCircle, XCircle, ExternalLink, Download } from 'lucide-react';
...
interface Job {
  ...
  experience: string;
  doj: string;
  status: 'Draft' | 'Pending Approval' | 'Approved' | 'In Progress' | 'Filled' | 'Cancelled';
  candidates: number;
  createdAt: string;
  updatedAt: string;
  jiraTicket?: string;
...
interface Candidate {
  ...
  phone: string;
  experience: string;
  status: 'Applied' | 'Screening' | 'Interview' | 'Selected' | 'Rejected' | 'Offered' | 'Joined';
  appliedDate: string;
  jobId: string;
}
...
export default function JobManagement() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'Filled': return <CheckCircle className="w-4 h-4 text-green-600" />;
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div><span className="font-medium">Expected DOJ:</span> {job.doj}</div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div><span className="font-medium">Created:</span> {job.createdAt}</div>
              <div><span className="font-medium">Last Updated:</span> {job.updatedAt}</div>
              <div><span className="font-medium">Candidates:</span> {job.candidates}</div>
              {job.jiraTicket && (
        ...
        <div>
          ...
          <div className="space-y-2">
            {candidates.filter(c => c.jobId === job.id).map(candidate => (
              <div key={candidate.id} className="flex items-center justify-between p-3 border rounded-lg">
                <div>
                  <div className="font-medium">{candidate.name}</div>
                  <div className="text-sm text-gray-500">{candidate.email} • {candidate.experience} experience</div>
                </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600">Manage all job postings and hiring workflows</p>
          </div>
          <Link to="/jobs/create">
            <Button>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{jobs.filter(j => ['Approved', 'In Progress'].includes(j.status)).length}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold">{jobs.filter(j => j.status === 'Pending Approval').length}</p>
                </div>
        ...
        <TabsContent value="candidates" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Track all candidates across active job postings</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
        ...
        <TabsContent value="approvals" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage job posting approvals and workflow status</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {jobs.filter(job => job.status === 'Pending Approval' || job.status === 'Draft').map((job) => (
                  <div key={job.id} className="p-4 border rounded-lg">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium">{job.title}</h3>
                        <p className="text-sm text-gray-500">{job.id} • {job.department} • HOD: {job.hod}</p>
                      </div>
